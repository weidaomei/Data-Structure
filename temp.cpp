...
public:
    void zc(User *U, Log a, Reader *R)
    {
        ...
        cin >> c;
        if (c == "y")
            R[a.number].Is_teacher = true;
        else
            R[a.number].Is_teacher = false;
        cout << "注册成功！" << endl;
...
void Admin::Librarian_Alter(Librarian *L, User *U, Admin *A, Reader *R)
{
    ...
    if (flag)
    {
        ...
        if (t == 1 || t == 2)
        {
            ...
            {
                if (j == Librarian::Librarian_Number - 1)
                {
